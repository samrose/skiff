...
**Tech stack:**
- Go 1.23+, modules under one `github.com/skiff-build/skiff` root, multiple `cmd/` binaries.
...
├── nix/
│   ├── flake.nix                     # dev shell + packager output
│   ├── flake.lock
│   └── packager.nix                  # the unpack-only derivation
├── deploy/
...
- Create: `nix/flake.nix`, `nix/flake.lock` (generated)
...
go mod init github.com/skiff-build/skiff
git add go.mod
git commit -s -m "feat: initialize go module"
```

(Use `-s` from this commit onward — DCO sign-off is enforced.)
...
- [ ] **0.3 Write `nix/flake.nix` with dev shell**

```nix
{
  description = "Skiff — npm hermetic packager";
  inputs = {
      ...
      in {
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            ...
            echo "skiff dev shell — Go $(go version | awk '{print $3}')"
          '';
        };
...
Smoke test:
```bash
nix develop --extra-experimental-features "nix-command flakes" --command go version
```
Expected: prints Go 1.23.x. Commit.
...
git add LICENSE README.md CONTRIBUTING.md CODE_OF_CONDUCT.md SECURITY.md \
        docs/ deploy/ nix/ .github/ .gitignore .golangci.yml
